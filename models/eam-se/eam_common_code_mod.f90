module eam_common_code_mod

! This module contains only code that is used by both the eam-fv and eam-se model mods.
...
public :: above_ramp_start, are_damping, build_eam_pressure_columns, build_heights, &
          eam_grid, cdebug_level, check_good_levels, cno_normalization_of_scale_heights, &
          pert_model_copies, cuse_log_vertical_scale, discarding_high_obs, &
          free_eam_grid, free_std_atm_tables, generic_height_to_pressure, &
...
character(len=256), parameter :: source   = 'eam_common_code.f90'
...
! Note that the eam_grid type contains information about staggered grids that are only for FV
! Note that the eam_grid type contains information for seperature dynamics and physics grid
type eam_grid
   type(eam_1d_array) :: lon
   type(eam_1d_array) :: lat
   type(eam_1d_array) :: plon
   type(eam_1d_array) :: plat
   type(eam_1d_array) :: slon
   type(eam_1d_array) :: slat
   type(eam_1d_array) :: lev
   type(eam_1d_array) :: ilev
   type(eam_1d_array) :: gw
   type(eam_1d_array) :: hyai
   type(eam_1d_array) :: hybi
   type(eam_1d_array) :: hyam
   type(eam_1d_array) :: hybm
   type(eam_1d_array) :: P0
end type

type(eam_grid) :: grid_data
...
! Value from namelist in model_mod EAM
...
!-----------------------------------------------------------------------
!> Read the data from the various eam grid arrays 
!>
...
subroutine get_eam_grid(grid_file, l_use_pgrid)
character(len=*), intent(in)  :: grid_file
logical,          intent(in)  :: l_use_pgrid

character(len=*), parameter :: routine = 'get_eam_grid:'
...
if ( l_use_pgrid ) then
  !SENotes: uses different grid for physics and dynamics
  write(string1, *) 'Cubed sphere grid is configured with different physics grid, read lat_d and lon_d instead'
  call error_handler(E_MSG, 'static_init_model', string1,source,revision,revdate)
  call fill_eam_1d_array(ncid, 'lon',   grid_data%plon)
  call fill_eam_1d_array(ncid, 'lat',   grid_data%plat)
  call fill_eam_1d_array(ncid, 'lon_d', grid_data%lon)
  call fill_eam_1d_array(ncid, 'lat_d', grid_data%lat)
else 
  !SENotes: when dynamics and physics grids are same, lat and lon are for both dynamics and physics grid 
  call fill_eam_1d_array(ncid, 'lon',   grid_data%lon)
  call fill_eam_1d_array(ncid, 'lat',   grid_data%lat)
  call fill_eam_1d_array(ncid, 'lon_d', grid_data%plon)
  call fill_eam_1d_array(ncid, 'lat_d', grid_data%plat)
end if

call fill_eam_1d_array(ncid, 'lev',   grid_data%lev)
call fill_eam_1d_array(ncid, 'ilev',  grid_data%ilev) ! for staggered vertical grid
call fill_eam_1d_array(ncid, 'slon',  grid_data%slon)
call fill_eam_1d_array(ncid, 'slat',  grid_data%slat)
call fill_eam_1d_array(ncid, 'gw',    grid_data%gw)   ! gauss weights
call fill_eam_1d_array(ncid, 'hyai',  grid_data%hyai)
call fill_eam_1d_array(ncid, 'hybi',  grid_data%hybi)
call fill_eam_1d_array(ncid, 'hyam',  grid_data%hyam)
call fill_eam_1d_array(ncid, 'hybm',  grid_data%hybm)

! P0 is a scalar with no dimensionality
call fill_eam_0d_array(ncid, 'P0',   grid_data%P0)

call nc_close_file(ncid, routine)

end subroutine get_eam_grid
...
subroutine fill_eam_1d_array(ncid, varname, grid_array)
integer,            intent(in)    :: ncid
character(len=*),   intent(in)    :: varname
type(eam_1d_array), intent(inout) :: grid_array

character(len=*), parameter :: routine = 'fill_eam_1d_array'

! SENote: For the SE core, three of these don't exist (gw, slon, slat) so need to check
! SENote: For the SE core, physics can use a different grid so lat_d and lon_d should be read in this case
...
end subroutine fill_eam_1d_array
...
subroutine free_eam_grid(grid)

type(eam_grid), intent(inout) :: grid

call free_eam_1d_array(grid%lon)
call free_eam_1d_array(grid%lat)
call free_eam_1d_array(grid%plon)
call free_eam_1d_array(grid%plat)
call free_eam_1d_array(grid%lev)
call free_eam_1d_array(grid%ilev)
call free_eam_1d_array(grid%slon)
call free_eam_1d_array(grid%slat)
call free_eam_1d_array(grid%gw)
call free_eam_1d_array(grid%hyai)
call free_eam_1d_array(grid%hybi)
call free_eam_1d_array(grid%hyam)
call free_eam_1d_array(grid%hybm)
call free_eam_1d_array(grid%P0)

end subroutine free_eam_grid
...
call nc_add_global_attribute(ncid, "model", "EAM", routine)
...
!----------------------------------------------------------------------------
if(grid_data%plon%nsize > 0) then !physics grid appear 
  call nc_define_dimension(ncid, 'lon',   grid_data%plon%nsize, routine)
  call nc_define_dimension(ncid, 'lon_d', grid_data%lon%nsize,  routine)
else
  call nc_define_dimension(ncid, 'lon',   grid_data%lon%nsize,  routine)
end if 

if(grid_data%plat%nsize > 0) then !physics grid appear 
  call nc_define_dimension(ncid, 'lat',   grid_data%plat%nsize, routine)
  call nc_define_dimension(ncid, 'lat_d', grid_data%lat%nsize,  routine)
else 
...
if(grid_data%plon%nsize > 0) then !physics grid appear 
  call nc_define_real_variable(     ncid, 'lon_d', (/ 'lon_d' /),               routine)
  call nc_add_attribute_to_variable(ncid, 'lon_d', 'long_name', 'longitude',    routine)
  call nc_add_attribute_to_variable(ncid, 'lon_d', 'units',     'degrees_east', routine)
end if 
...
if(grid_data%plat%nsize > 0) then !physics grid appear 
  call nc_define_real_variable(     ncid, 'lat_d', (/ 'lat_d' /),                routine)
  call nc_add_attribute_to_variable(ncid, 'lat_d', 'long_name', 'latitude',      routine)
  call nc_add_attribute_to_variable(ncid, 'lat_d', 'units',     'degrees_north', routine)
end if 
...
!SE-PG2Note:pg2 grid has extra dimention 
if(grid_data%plon%nsize > 0) then !physics grid appear 
  call nc_put_variable(ncid, 'lon_d', grid_data%lon%vals,  routine)
  call nc_put_variable(ncid, 'lon',   grid_data%plon%vals, routine)
else 
  call nc_put_variable(ncid, 'lon', grid_data%lon%vals,  routine)
end if 

if(grid_data%plat%nsize > 0) then !physics grid appear 
  call nc_put_variable(ncid, 'lat_d', grid_data%lat%vals,  routine)
  call nc_put_variable(ncid, 'lat',   grid_data%plat%vals, routine)
else 
...
subroutine free_eam_1d_array(grid_array)
type(eam_1d_array), intent(inout) :: grid_array

deallocate(grid_array%vals)
grid_array%nsize = -1

end subroutine free_eam_1d_array
...
subroutine fill_eam_0d_array(ncid, varname, grid_array)
integer,            intent(in)    :: ncid
character(len=*),   intent(in)    :: varname
type(eam_1d_array), intent(inout) :: grid_array

character(len=*), parameter :: routine = 'fill_eam_0d_array'
...
end subroutine fill_eam_0d_array
...
! is enabled (usually true for eam).
...
! eam uses a uniform gas constant value, but high top
...
if(grid_data%hybi%vals(nlevels + 1) /= 1.0_r8) then
   call error_handler(E_ERR, 'build_heights in eam_common_code_mod.f90', &
...
!-----------------------------------------------------------------------
!> writes EAM's model date and time of day into file.  EAM uses
...
eam_date = iyear*10000 + imonth*100 + iday
eam_tod  = ihour*3600  + iminute*60 + isecond
...
if (.not. nc_variable_exists(ncid, "date")) then
   ...
   call nc_end_define_mode(ncid, routine)
   call nc_put_variable(ncid, 'date', eam_date, routine)
endif
...
if (.not. nc_variable_exists(ncid, "datesec")) then
   ...
   call nc_end_define_mode(ncid, routine)
   call nc_put_variable(ncid, 'datesec', eam_tod,  routine)
endif
...
! EAM initial files have two variables of length 
! 'time' (the unlimited dimension): date, datesec

call nc_get_variable(ncid, 'date',    eam_date, routine)
call nc_get_variable(ncid, 'datesec', eam_tod,  routine)

! 'date' is YYYYMMDD 
! 'eam_tod' is seconds of current day
iyear  = eam_date / 10000
rem    = eam_date - iyear*10000
imonth = rem / 100
iday   = rem - imonth*100

ihour  = eam_tod / 3600
rem    = eam_tod - ihour*3600
imin   = rem / 60
isec   = rem - imin*60

! some eam files are from before the start of the gregorian calendar.
...
!--------------------------------------------------------------------
!> in eam level 1 is at the model top, level N is the lowest level
...
! eam levels start at the top so level 1 is
...
subroutine build_eam_pressure_columns(ens_size, surface_pressure, n_levels, pressure_array)
...
! eam model levels: 1 is the model top, N is the bottom.

do j=1, ens_size
   call single_pressure_column(surface_pressure(j), n_levels, pressure_array(:, j))
enddo

end subroutine build_eam_pressure_columns
...
! eaminput.nc files. Suspect that a few of the hyam's are bad, but it could also be
...
subroutine read_grid_info(grid_file, l_use_pgrid)
character(len=*), intent(in)  :: grid_file  ! eam template file
logical,          intent(in)  :: l_use_pgrid

! Get the grid info plus additional non-state arrays
call get_eam_grid(grid_file, l_use_pgrid)

end subroutine read_grid_info

!========================================================================

end module eam_common_code_mod
