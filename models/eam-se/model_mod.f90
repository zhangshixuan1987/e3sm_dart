...
!----------------------------------------------------------------
!>
!> this is the interface between the eam-se atmosphere model and dart.
...
! examples of additional quantities that eam-chem might need defined from the obs_kind_mod
!                                   ! GASES
...
use    eam_common_code_mod, only : above_ramp_start, are_damping, build_eam_pressure_columns, build_heights, &
...
use eam_common_code_mod, only : nc_write_model_atts, grid_data, read_grid_info, &
                                MAX_PERT, &
...
character(len=*), parameter :: source   = 'eam-se/model_mod.f90'
...
character(len=256) :: eam_template_filename           = 'eaminput.nc'
character(len=256) :: eam_phis_filename               = 'eam_phis.nc'
logical            :: eam_use_pgrid                   = .false.
...
real(r8), allocatable :: phis(:), phis_d(:)
...
! ne = global metadata from eam_template_filename, giving the number of 'elements' per face edge of the 'cube'.
! np = # of nodes/edge of each element.  Edges are shared with adjacent elements.
!fv_nphys = # of physics grid. fv_nphys = 2 for pg2 grid.  
! FIXME? put these in a derived type to prevent accidentally using them as local variables.
integer :: ne, np, fv_nphys
...
! Or derive from l_refined, after ne is read from eaminput.nc?
...
! EAM fields and sizes in the init code.
...
! Set values from namelist in eam_common_code_mod
...
call read_grid_info(eam_template_filename, eam_use_pgrid)
! This non-state variable is used to compute surface elevation.
call read_eam_phis_array(eam_phis_filename, eam_use_pgrid)
...
domain_id = add_domain(eam_template_filename, parse_variables_clamp(state_variables))
...
!write(string1, *) 'Size of surface pressure, ncol = ', ncol_temp(1) 
...
nc_file_ID = nc_open_file_readonly(eam_template_filename, 'Reading ne and np from eam template file')
...
!if (print_details) then
!   write(string1, *) 'file = ', cs_grid_file, 'status = ', file_exist(cs_grid_file)
!   call error_handler(E_MSG, 'static_init_model', string1,source,revision,revdate)
!   write(string1, *) 'file = ', homme_map_file, 'status = ', file_exist(homme_map_file)
!   call error_handler(E_MSG, 'static_init_model', string1,source,revision,revdate)
!endif
...
elseif (file_exist(homme_map_file)) then
   ...
   !if (print_details) then 
   !  write(string1,*) 'create_cs_grid_arrays done, write out the data '
   !  call error_handler(E_MSG, 'create_cs_grid_arrays', string1, source, revision, revdate)
   !end if 
   if (my_task_id() == 0) call nc_write_cs_grid_file( trim(cs_grid_file), trim(homme_map_file) )
else
...
select case (obs_quantity) 
   case (QTY_PRESSURE)
      call eam_se_pressure_levels(ens_handle, ens_size, column, ref_nlevels, &
...
select case (which_vert)
   ...
   case(VERTISPRESSURE)
      ...
      if(dry_mass_vertical_coordinate .and. precise) then
         ...
         call build_eam_pressure_columns(ens_size, surf_pressure, ref_nlevels, pressure_array)
      endif
   ...
   case(VERTISHEIGHT)
      ...
      call eam_se_height_levels(ens_handle, ens_size, column, ref_nlevels, &
...
! Note that other tracers in the dry mass eam/se have a dry mixing ratio, not specific mixing ratio (although
! Peter plans to change this for consistency).

!SENote: The following should all be 0, but A's at levels 2, 3, 4, and 5 are not.
! This is confirmed in the eaminput.nc file and is a PROBLEM.
...
subroutine eam_se_height_levels(ens_handle, ens_size, column, nlevels, precise, height_array, my_status) 
type(ensemble_type), intent(in)  :: ens_handle
integer,             intent(in)  :: ens_size
integer,             intent(in)  :: column
integer,             intent(in)  :: nlevels
...
if(dry_mass_vertical_coordinate .and. precise) then
   ...
   call build_eam_pressure_columns(ens_size, surface_pressure, nlevels, pressure)
endif
...
end subroutine eam_se_height_levels
...
subroutine read_eam_phis_array(phis_filename, l_use_pgrid)
character(len=*),   intent(in)    :: phis_filename
logical,            intent(in)    :: l_use_pgrid

character(len=*), parameter :: routine = 'read_eam_phis_array'
...
if (l_use_pgrid) then
  ...
  write(string1, *) 'This is a pg2 grid topo as PHIS_d appears, read PHIS_d instead'
  call error_handler(E_MSG,routine,string1,source,revision,revdate)

  call nc_get_variable_size(ncid, 'PHIS_d', nsize(:), routine)

  write(string1, *) 'Size of PHIS_d, nsize(1:3) = ', nsize(1), nsize(2), nsize(3)
  call error_handler(E_MSG,routine,string1,source,revision,revdate)

  allocate( phis(nsize(1)))

  call nc_get_variable(ncid, 'PHIS_d', phis, routine)
...
end subroutine read_eam_phis_array
...
subroutine eam_se_pressure_levels(ens_handle, ens_size, column, nlevels, &
                               precise, pressure_array, my_status) 
type(ensemble_type), intent(in)  :: ens_handle
integer,             intent(in)  :: ens_size
integer,             intent(in)  :: column
...
if(dry_mass_vertical_coordinate .and. precise) then
   ...
   call build_eam_pressure_columns(ens_size, surface_pressure, ref_nlevels, pressure_array)
endif

! No error returns available if we get here, so all good
my_status(:) = 0

end subroutine eam_se_pressure_levels
...
if (no_normalization_of_scale_heights) then
   ...
   if (query_location(location) == VERTISSURFACE) then
      ...
      !> that location.  that's what the original eam model_mod did.  is that ok?
...
if (file_exist(homme_map_file)) then
   nc_file_ID = nc_open_file_readonly(trim(homme_map_file))
   ...
   write(string1, *) 'EAM-SE grid file "',trim(homme_map_file),'" found, with ncenters, ncorners =  ', ncenters, ncorners 
   call error_handler(E_MSG,'create_cs_grid_arrays',string1,source,revision,revdate)

else
   write(string1, *) 'EAM-SE grid file "',trim(homme_map_file),'" can not be found '
   call error_handler(E_ERR,'create_cs_grid_arrays',string1,source,revision,revdate)
endif
...
if (local_ncol /= ncol) then
   write(string1, *) trim(cs_grid_file),' ncol in cs_grid_file does not match the one in eaminput.nc', &
         local_ncol, ncol
   call error_handler(E_ERR,'nc_read_cs_grid_file',string1,source,revision,revdate)
endif
...
call nc_add_global_attribute(nc_file_ID, 'HommeMapping_file', trim(homme_map_file))
...
! No return is needed.  A diagnostic statement is printed before a potential
! failure
...
! PS is required for both eam-se and eam-fv, 
