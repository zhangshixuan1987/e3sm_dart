...
DART_NNODES=${my_eam_dart_nnodes}
DART_NTASKS=$((DART_NNODES * my_task_per_node))
...
for required_time_var in DART_YEAR DART_MONTH DART_DAY DART_SECONDS; do
  if [[ -z "${!required_time_var:-}" || ! "${!required_time_var}" =~ ^[0-9]+$ ]]; then
    echo "ERROR: invalid or unset DART time variable ${required_time_var}: ${!required_time_var:-unset}"
    exit 42
  fi
done
if (( DART_YEAR < 1 || DART_MONTH < 1 || DART_MONTH > 12 || DART_DAY < 1 || DART_DAY > 31 || DART_SECONDS < 0 || DART_SECONDS >= 86400 )); then
  echo "ERROR: DART valid time is out of range: ${DART_YEAR}-${DART_MONTH}-${DART_DAY} ${DART_SECONDS}s"
  exit 43
fi
# ==============================================================================
...
#=========================================================================
# Link the archived forecast files to DART's required member names. filter
# updates these files in place through the links, which avoids a second full
# ensemble on disk. Pre/post validation and an in-progress marker detect an
# interrupted or incomplete assimilation, but cannot roll it back.
#=========================================================================
if [ ${my_ensnum} -eq 1 ]; then
  ...
  echo "Linking ${ATM_INITIAL_FILENAME} as DART member ${ATM_DART_FILENAME}"
...
if [[ -n "${cycle_cutoff}" ]]; then
  cutoff="${cycle_cutoff}"
  echo "Using configured localization cutoff override for ${ATM_DATE_EXT}: ${cutoff}"
fi
if [[ -n "${cycle_inf_damping}" ]]; then
  inf_damping="${cycle_inf_damping}"
  echo "Using configured inflation damping override for ${ATM_DATE_EXT}: ${inf_damping}"
fi
if [[ "${eam_no_obs_assim_above_level}" != "${my_eam_no_obs_assim_above_level}" ]]; then
  echo "Using configured upper-level observation cutoff override for ${ATM_DATE_EXT}: ${eam_no_obs_assim_above_level}"
fi

# Optional per-job overrides keep exceptional-cycle recovery on the normal,
# validated workflow instead of requiring a separate copy of this script.
cutoff=${DART_CUTOFF_OVERRIDE:-${cutoff}}
inf_damping=${DART_INF_DAMPING_OVERRIDE:-${inf_damping}}
if ! [[ "${cutoff}" =~ ^[0-9]+([.][0-9]+)?$ && "${inf_damping}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
  echo "ERROR: invalid DART cutoff/damping: cutoff=${cutoff}, inf_damping=${inf_damping}"
  exit 44
fi
echo "DART localization/inflation settings: cutoff=${cutoff}, inf_damping=${inf_damping}"

if ! [[ "${eam_no_obs_assim_above_level:-}" =~ ^[1-9][0-9]*$ ]] || (( eam_no_obs_assim_above_level > 72 )); then
  echo "ERROR: my_eam_no_obs_assim_above_level must be a model-level index from 1 through 72, got: ${eam_no_obs_assim_above_level:-unset}"
  exit 44
fi
echo "DART upper-level observation exclusion: no_obs_assim_above_level=${eam_no_obs_assim_above_level}"
if ! [[ "${my_eam_use_log_vertical_scale:-}" =~ ^\.(true|false)\.$ ]]; then
  echo "ERROR: my_eam_use_log_vertical_scale must be .true. or .false., got: ${my_eam_use_log_vertical_scale:-unset}"
  exit 44
fi
if ! [[ "${my_eam_vert_normalization_scale_height:-}" =~ ^[0-9]+([.][0-9]+)?$ ]] || ! awk -v value="${my_eam_vert_normalization_scale_height}" 'BEGIN {exit !(value > 0)}'; then
  echo "ERROR: my_eam_vert_normalization_scale_height must be positive, got: ${my_eam_vert_normalization_scale_height:-unset}"
  exit 44
fi
echo "DART vertical localization: use_log_vertical_scale=${my_eam_use_log_vertical_scale}, vert_normalization_scale_height=${my_eam_vert_normalization_scale_height}"
...
if [ -e "${my_eam_filter_nml}" ]; then
  ${COPY} "${my_eam_filter_nml}" input.nml  || exit 04
  sed -i "/#/d;/^\!/d;/^[ ]*\!/d" input.nml
  sed -i '1,1i\WARNING: Changes to this runtime file will be ignored. \n Edit workflow_lib/namelists/eam/filter.nml instead.\n\n\n' input.nml
else
  echo "ERROR ... workflow EAM filter namelist ${my_eam_filter_nml} not found ... ERROR"
  echo "ERROR ... workflow EAM filter namelist ${my_eam_filter_nml} not found ... ERROR"
  exit 05
fi
...
g;use_log_vertical_scale ;s;= .*;= ${my_eam_use_log_vertical_scale};
g;vert_normalization_scale_height ;s;= .*;= ${my_eam_vert_normalization_scale_height};
g;no_obs_assim_above_level ;s;= .*;= ${eam_no_obs_assim_above_level};
g;cutoff ;s;= .*;= ${cutoff};
g;inf_damping ;s;= .*;= ${inf_damping}, ${inf_damping};
wq
ex_end

grep -Eq "^[[:space:]]*no_obs_assim_above_level[[:space:]]*=[[:space:]]*${eam_no_obs_assim_above_level}([[:space:]]|$)" input.nml || {
  echo "ERROR: failed to configure no_obs_assim_above_level in runtime input.nml"
  exit 44
}

grep -Eq "^[[:space:]]*use_log_vertical_scale[[:space:]]*=[[:space:]]*${my_eam_use_log_vertical_scale}([[:space:]]|$)" input.nml || {
  echo "ERROR: failed to configure use_log_vertical_scale in runtime input.nml"
  exit 44
}
grep -Eq "^[[:space:]]*vert_normalization_scale_height[[:space:]]*=[[:space:]]*${my_eam_vert_normalization_scale_height}([[:space:]]|$)" input.nml || {
  echo "ERROR: failed to configure vert_normalization_scale_height in runtime input.nml"
  exit 44
}
...
# Configure round-robin I/O layout using the validated scalar task density.
if [[ ! "${my_task_per_node:-}" =~ ^[1-9][0-9]*$ ]]; then
   echo "ERROR: my_task_per_node must be a positive integer" >&2
...
DART_IN_PROGRESS_MARKER="${DA_TRANSACTION_DIR}/.dart_filter_in_progress"
mkdir -p "${DA_TRANSACTION_DIR}" || exit 46
if [ -e "${DART_IN_PROGRESS_MARKER}" ]; then
  echo "ERROR: found stale DART in-progress marker: ${DART_IN_PROGRESS_MARKER}"
  echo "ERROR: the archived ensemble may contain partially updated members; inspect it before retrying"
  exit 45
fi
printf 'cycle=%s\nvalid_time=%s\nslurm_job_id=%s\nstarted_at=%s\n' "${DATA_ASSIMILATION_CYCLES}" "${ATM_DATE_EXT}" "${SLURM_JOB_ID:-none}" "$(date '+%Y-%m-%d %H:%M:%S')" > "${DART_IN_PROGRESS_MARKER}" || exit 46

echo "`date` -- BEGIN FILTER"
${LAUNCHCMD} ${CURRENT_DADIR}/filter || exit 34
echo "`date` -- END FILTER"

# The symlink targets have already been updated in place. Validate the complete
# ensemble before allowing cycle handoff, then clear the durable failure marker.
echo "`date` -- BEGIN ANALYSIS VALIDATION"
for i in `seq 1 ${DART_ENSNUM}`; do
  inst_string=`printf _%04d ${i}`
  ENSTR=EN`printf "%02d" ${i}`
  analysis_file="${CURRENT_DADIR}/${DART_CASE}.eam${inst_string}.i.${ATM_DATE_EXT}.nc"

  if [ ! -s "${analysis_file}" ] || ! ncdump -h "${analysis_file}" >/dev/null 2>&1; then
    echo "ERROR: invalid DART analysis for ${ENSTR}: ${analysis_file}"
    exit 47
  fi
  if command -v ncks >/dev/null 2>&1 && ! ncks -m -v PS,U,V,T,Q "${analysis_file}" >/dev/null 2>&1; then
    echo "ERROR: DART analysis is missing core EAM variables for ${ENSTR}: ${analysis_file}"
    exit 48
  fi
done
rm -f "${DART_IN_PROGRESS_MARKER}" || exit 49
echo "Validated ${DART_ENSNUM} in-place analysis members"
echo "`date` -- END ANALYSIS VALIDATION"
...
for stage in ${stages_all[@]}; do
  for FILE in `${LIST} ${stage}_member_*.nc` ; do
    ...
    ${MOVE} $FILE \
        ${DART_CASE}.${scomp}_${list[${#list[@]}-1]}.${type}.${dart_file}.${ATM_DATE_EXT} || exit 44
    ${REMOVE} dart_tmp.txt
  done
done
