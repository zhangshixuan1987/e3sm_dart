module EcosystemBalanceCheckMod
  ...
  !
  ! DART note: skip balance checks for the first restart step after assimilation.
  ! The DA algorithm may create/destroy mass; no reason to check balance on startup.
  !
  ! !USES:
  ...
  use elm_time_manager    , only : get_step_size, get_nstep, is_first_restart_step
...
#ifndef _OPENACC
       if (err_found .and. nstep > 1 .and. (.not. is_first_restart_step())) then
          c = err_index
  ...
  subroutine GridCBalanceCheck(bounds, col_cs, col_cf, grc_cs, grc_cf)
    ...
    associate(                                                       &
      ...
      do g = bounds%begg, bounds%endg
         ...
         if (abs(grc_errcb(g)) > balance_check_tolerance  .and. nstep > 1 .and. (.not. is_first_restart_step())) then
