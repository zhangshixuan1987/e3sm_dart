module SurfaceRadiationMod
  ...
  type, public :: surfrad_type
     ...
     real(r8), pointer  :: parveg_patch          (:) => null() ! patch  absorbed par by vegetation (W/m**2)
...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    allocate(this%parveg_patch          (begp:endp))              ; this%parveg_patch          (:)   = spval
  ...
  subroutine InitHistory(this, bounds)
    ...
    this%parveg_patch(begp:endp) = spval
    call hist_addfld1d (fname='PARVEG', units='W/m^2',  &
         avgflag='A', long_name='absorbed par by vegetation', &
         ptr_patch=this%parveg_patch)
  ...
  subroutine SurfaceRadiation(bounds, num_nourbanp, filter_nourbanp, &
     ...
     associate(                                                     &
          ...
          parveg          =>    surfrad_vars%parveg_patch         , & ! Output: [real(r8) (:)   ] absorbed par by vegetation (W/m**2)
       ...
       do ib = 1, nband
          do fp = 1,num_nourbanp
             ...
             if (ib == 1) then
                ...
                surfrad_vars%parveg_patch(p) = parveg(p)
             end if
             if (lun_pp%itype(l)==istsoil .or. lun_pp%itype(l)==istcrop) then
