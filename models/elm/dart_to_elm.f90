...
!----------------------------------------------------------------------
! purpose: Update the ELM restart file with the DART posterior. If need
...
!         ELM restart file. Anything with a DART posterior _FillValue 
!         is replaced with the original ELM value. 
!
! discussion: Some variables in the ELM restart file have variables that
!         have neither the declared _FillValue nor a predictable value.
!         The 'elm_to_dart' program replaced those ELM indeterminate 
...
!         _FillValue with whatever was originally in the ELM restart file.
!       
...
character(len=*), parameter :: source = 'dart_to_elm.f90'

!------------------------------------------------------------------
! The namelist variables
!------------------------------------------------------------------

character(len=256) :: dart_to_elm_input_file = 'dart_posterior.nc'
character(len=256) :: dart_to_elm_output_file = 'elm_restart.nc'
integer            :: repartition_swe = 0
character(len=256) :: repartition_vhist_file = 'elm_vector_history.nc'
character(len=256) :: repartition_analysis_file = 'dart_posterior_vector.nc'
integer            :: verbose = 0

namelist /dart_to_elm_nml/ dart_to_elm_input_file, &
                           dart_to_elm_output_file, &
                           repartition_swe, &
...
integer            :: ncid_dart, ncid_elm
type(time_type)    :: dart_time, elm_time

character(len=512) :: string1, string2, string3
character(len=NF90_MAX_NAME) :: varname
!======================================================================

call initialize_utilities(progname='dart_to_elm')

! Call model_mod:static_init_model() to reads the elm namelist

call static_init_model()

! Read the namelist to get the input/output filenames. 

call find_namelist_in_file("input.nml", "dart_to_elm_nml", iunit)
read(iunit, nml = dart_to_elm_nml, iostat = io)
call check_namelist_read(iunit, io, "dart_to_elm_nml")

! Record the namelist values used for the run
if (do_nml_file()) write(nmlfileunit, nml=dart_to_elm_nml)
if (do_nml_term()) write(     *     , nml=dart_to_elm_nml)

ncid_dart = nc_open_file_readonly( dart_to_elm_input_file,  'opening to read posterior')
ncid_elm  = nc_open_file_readwrite(dart_to_elm_output_file, 'opening to write restart')

write(string1,*)'reading updated values from DART file "'//trim(dart_to_elm_input_file)//'"'
write(string2,*)'and writing to ELM restart file "'//trim(dart_to_elm_output_file)//'"'
call error_handler(E_MSG,source,string1,text2=string2)

! Make sure we are updating the right restart file - 

dart_time = read_model_time(dart_to_elm_input_file)
elm_time  = read_model_time(dart_to_elm_output_file)

if (dart_time /= elm_time) then
   write(string1,*)'DART timestamp "'//trim(dart_to_elm_input_file)//'" /= '
   write(string2,*)'ELM  timestamp "'//trim(dart_to_elm_output_file)//'"'
   call error_handler(E_MSG,source,string1,text2=string2)
endif

call print_date( elm_time,'dart_to_elm:elm  model date')
call print_time( elm_time,'dart_to_elm:DART model time')
call print_date( elm_time,'dart_to_elm:elm  model date',logfileunit)
call print_time( elm_time,'dart_to_elm:DART model time',logfileunit)

! update the current elm restart file
!
  ...
  if (repartition_swe > 0) then
   ...
   select case (varname)
      case ('SNOWDP', 'SNOW_DEPTH', 'DZSNO', 'ZSNO', 'ZISNO', 'H2OSOI_LIQ', 'H2OSOI_ICE')
         ...
         call error_handler(E_MSG, 'dart_to_elm', string1, text2=string2)
         cycle UPDATE
      case default
   ...
   if (rank == 1) then
      call replace_values_1D(dom_restart, ivar, ncid_dart, ncid_elm)

   elseif (rank == 2) then
      call replace_values_2D(dom_restart, ivar, ncid_dart, ncid_elm)
...
enddo UPDATE
 ...
 ! Pass in the soil/snow and column dimensions to update_snow subroutine
     if (nc_variable_exists(ncid_elm, 'H2OSOI_ICE')) then
        call nc_get_variable_size(ncid_elm, 'H2OSOI_ICE', ICE_varsize, source)
        nlevsno = nc_get_dimension_size(ncid_elm, 'levsno')
     else
        write(string1,*)'Snow repartitioning requires "H2OSOI_ICE" variable'
        call error_handler(E_ERR,source,string1,source)
     endif
     
     call update_snow(dom_restart, ncid_dart, ncid_elm, ICE_varsize(2), &
...
call nc_close_file(ncid_elm,  source)
call nc_close_file(ncid_dart, source)

! Log what we think we're doing, and exit.

call finalize_utilities('dart_to_elm')
...
!> the ELM declared '_FillValue' and the clamping values have been honored. 
!> Get the 'original' variable from the netcdf file.

subroutine replace_values_1D(dom_id, ivar, ncid_dart, ncid_elm)

integer, intent(in) :: dom_id
integer, intent(in) :: ivar
integer, intent(in) :: ncid_dart
integer, intent(in) :: ncid_elm

character(len=*), parameter :: routine = 'replace_values_1D'

character(len=NF90_MAX_NAME) :: varname
real(r8), allocatable :: dart_array(:)
real(r8), allocatable :: elm_array(:)
real(r8) :: special
integer  :: varsize(1)
...
call Compatible_Variables(varname, ncid_dart, ncid_elm, varsize)

allocate(dart_array(varsize(1)), elm_array(varsize(1)))

call nc_get_variable(ncid_elm,  varname,  elm_array)
call nc_get_variable(ncid_dart, varname, dart_array)

where(dart_array /= special) elm_array = dart_array

call nc_put_variable(ncid_elm, varname, elm_array, routine)

deallocate(dart_array, elm_array)
...
subroutine replace_values_2D(dom_id, ivar, ncid_dart, ncid_elm)
...
allocate(dart_array(varsize(1),varsize(2)), elm_array(varsize(1),varsize(2)))
...
subroutine update_snow(dom_id, ncid_dart, ncid_elm, ncolumn, nlevel, nlevsno, repartition_swe)
...
!> prognostic elm variables: SNOWDP/SNOW_DEPTH, DZSNO, H2OSOI_LIQ, H2OSOI_ICE
!>                           ZSNO and ZISNO
...
integer, intent(in) :: ncid_dart
integer, intent(in) :: ncid_elm
integer             :: ncid_elm_vector, ncid_dart_vector
...
real(r8) :: elm_H2OSNO(ncolumn)  !(column,time) for vector history
integer  :: elm_SNLSNO(ncolumn)
real(r8) :: elm_SNOWDP(ncolumn)
real(r8) :: elm_DZSNO(nlevsno,ncolumn)
real(r8) :: elm_ZSNO(nlevsno,ncolumn)
real(r8) :: elm_ZISNO(nlevsno,ncolumn)
real(r8) :: elm_H2OLIQ(nlevel,ncolumn)
real(r8) :: elm_H2OICE(nlevel,ncolumn)
...
! in DART state and ELM domain (restart,history,vector history)
...
! depth elm variables. The H2OSNO variable (vectory history) is 
! required for repartitioning.  There are mutliple 'snow depth' 
! variable names depending on elm version.
...
if (verbose > 0) then
   ...
   write(string1,*)'ELM domain 1 variable:  "'//trim(varname)//'"' 
   ...
   write(string1,*)'ELM domain 2 variable: "'//trim(varname)//'"' 
   call error_handler(E_MSG, routine, string1, source)
   enddo domain2
   ...
   write(string1,*)'ELM domain 3 variable: "'//trim(varname)//'"'
   call error_handler(E_MSG, routine, string1, source)
   enddo domain3
...
! BEGIN elm variable and DART state checks for snow repartitioning

if (nc_variable_exists(ncid_elm, 'SNLSNO') .and. nc_variable_exists(ncid_elm, 'DZSNO') .and. &
    nc_variable_exists(ncid_elm, 'H2OSOI_LIQ') .and. nc_variable_exists(ncid_elm, 'H2OSOI_ICE') &
    .and. nc_variable_exists(ncid_elm, 'ZSNO') .and. nc_variable_exists(ncid_elm, 'ZISNO')) then                             
    call nc_get_variable(ncid_elm, 'SNLSNO',  elm_SNLSNO)
    call nc_get_variable(ncid_elm, 'DZSNO',  elm_DZSNO)
    call nc_get_variable(ncid_elm, 'ZSNO',  elm_ZSNO)
    call nc_get_variable(ncid_elm, 'ZISNO',  elm_ZISNO)
    call nc_get_variable(ncid_elm, 'H2OSOI_LIQ',  elm_H2OLIQ)
    call nc_get_variable(ncid_elm, 'H2OSOI_ICE',  elm_H2OICE)
else
    write(string1,*)'Snow repartitioning requires elm variables: '
    write(string2,*)'SNLSNO, DZSNO, ZSNO, ZISNO, H2OSOI_LIQ, H2OSOI_ICE,'
    call error_handler(E_ERR,routine,string1,source,text2=string2)
endif

ncid_elm_vector= nc_open_file_readonly(repartition_vhist_file, &
                 'confirm H2OSNO is in elm_vector_history.nc')

if (nc_variable_exists(ncid_elm_vector, 'H2OSNO')) then
   call nc_get_variable(ncid_elm_vector, 'H2OSNO',  elm_H2OSNO)
else
   write(string1,*)'Snow repartitioning requires elm SWE variable'
   write(string2,*)'Check for H2OSNO within elm vector history'
   call error_handler(E_ERR,routine,string1,source,text2=string2)
endif

! Multiple options for elm snow depth variable
if (nc_variable_exists(ncid_elm, 'SNOW_DEPTH')) then
   call nc_get_variable(ncid_elm, 'SNOW_DEPTH',  elm_SNOWDP)
elseif (nc_variable_exists(ncid_elm, 'SNOWDP')) then
   call nc_get_variable(ncid_elm, 'SNOWDP',  elm_SNOWDP)
else
   write(string1,*)'Snow repartitioning requires elm snow depth variable'
...
! END elm variable and DART state checks

h2osno_pr=elm_H2OSNO
snlsno=elm_SNLSNO
snowdp_pr=elm_SNOWDP
dzsno_pr=elm_DZSNO
zsno_pr=elm_ZSNO
zisno_pr=elm_ZISNO
h2oliq_pr=elm_H2OLIQ
h2oice_pr=elm_H2OICE
gain_dzsno(:,:)= 0.0_r8

! Likely unnecessary check coming from elm restart file
...
PARTITION: do icolumn = 1,ncolumn
   ...
   if (h2osno_po(icolumn) < 0.0_r8) then  ! If there IS NOT snow in column...
      ...
      ! Leave layer aggregation/initialization to ELM
      ...
      if (snlsno(icolumn) < 0) then  ! If there IS snow in the column ...
         ...
         do c = 1,-snlsno(icolumn)   ! Identify total snow layers to repartition
            ...
            ! In ELM4 nlevsno=5, but in ELM5 nlevsno=12
            ...
            ! Important to update snow layer dimensions because ELM code relies
...
! Update the 'elm_array' with repartitioned 'dart_array' similar to 
...
if (nc_variable_exists(ncid_dart, 'SNOW_DEPTH')) then
   call Compatible_Variables('SNOW_DEPTH', ncid_dart, ncid_elm, varsize)
   where(dart_SNOWDP /= special) elm_SNOWDP = dart_SNOWDP
   call nc_put_variable(ncid_elm, 'SNOW_DEPTH', elm_SNOWDP, routine)
else
   call Compatible_Variables('SNOWDP', ncid_dart, ncid_elm, varsize)
   where(dart_SNOWDP /= special) elm_SNOWDP = dart_SNOWDP
   call nc_put_variable(ncid_elm, 'SNOWDP', elm_SNOWDP, routine)
endif
...
if (get_has_missing_value(dom_id,VarID)) call get_FillValue(    dom_id,VarID,special)
! Make sure variables in both files are identical in shape, etc.
call Compatible_Variables('DZSNO', ncid_dart, ncid_elm, varsize)
where(dart_DZSNO /= special) elm_DZSNO = dart_DZSNO
call nc_put_variable(ncid_elm, 'DZSNO', elm_DZSNO, routine)
...
if (get_has_missing_value(dom_id,VarID)) call get_FillValue(    dom_id,VarID,special)
! Make sure variables in both files are identical in shape, etc.
call Compatible_Variables('ZSNO', ncid_dart, ncid_elm, varsize)
where(dart_ZSNO /= special) elm_ZSNO = dart_ZSNO
call nc_put_variable(ncid_elm, 'ZSNO', elm_ZSNO, routine)
...
if (get_has_missing_value(dom_id,VarID)) call get_FillValue(    dom_id,VarID,special)
! Make sure variables in both files are identical in shape, etc.
call Compatible_Variables('ZISNO', ncid_dart, ncid_elm, varsize)
where(dart_ZISNO /= special) elm_ZISNO = dart_ZISNO
call nc_put_variable(ncid_elm, 'ZISNO', elm_ZISNO, routine)
...
if (get_has_missing_value(dom_id,VarID)) call get_FillValue(    dom_id,VarID,special)
! Make sure variables in both files are identical in shape, etc.
call Compatible_Variables('H2OSOI_LIQ', ncid_dart, ncid_elm, varsize)
where(dart_H2OLIQ /= special) elm_H2OLIQ = dart_H2OLIQ
call nc_put_variable(ncid_elm, 'H2OSOI_LIQ', elm_H2OLIQ, routine)
...
if (get_has_missing_value(dom_id,VarID)) call get_FillValue(    dom_id,VarID,special)
! Make sure variables in both files are identical in shape, etc.
call Compatible_Variables('H2OSOI_ICE', ncid_dart, ncid_elm, varsize)
where(dart_H2OICE /= special) elm_H2OICE = dart_H2OICE
call nc_put_variable(ncid_elm, 'H2OSOI_ICE', elm_H2OICE, routine)
...
subroutine Compatible_Variables(varname, ncid_dart, ncid_elm, varsize)
...
integer ::  elm_numdims,  elm_dimlengths(NF90_MAX_VAR_DIMS)
integer :: dart_numdims, dart_dimlengths(NF90_MAX_VAR_DIMS)

character(len=NF90_MAX_NAME) :: elm_dimnames( NF90_MAX_VAR_DIMS)
character(len=NF90_MAX_NAME) :: dart_dimnames(NF90_MAX_VAR_DIMS)

call nc_get_variable_num_dimensions( ncid_elm, varname, elm_numdims,    routine)
call nc_get_variable_dimension_names(ncid_elm, varname, elm_dimnames,   routine)
call nc_get_variable_size(           ncid_elm, varname, elm_dimlengths, routine)
...
if (elm_numdims /= dart_numdims) then
   write(string1,*)'Variable ranks not identical and must be.'
   write(string2,*)'ELM  "'//trim(varname)//'" has rank ',elm_numdims
...
DimCheck : do irank = 1,elm_numdims

   if ( elm_dimlengths(irank) /= dart_dimlengths(irank) ) then
      write(string1,*)'Variable dimension lengths not identical and must be.'
      write(string2,*)'ELM  "'//trim(varname)//'" dim=',irank, ', length=',elm_dimlengths(irank)
      write(string3,*)'DART "'//trim(varname)//'" dim=',irank, ', length=',dart_dimlengths(irank)
      call error_handler(E_ERR, routine, string1, source, text2=string2, text3=string3)
   endif

   ! Do we want to check dimension name too?
   if ( elm_dimnames(irank) /= dart_dimnames(irank) ) then
      write(string1,*)'Variable dimension names not identical and must be.'
      write(string2,*)'ELM  "'//trim(varname)//'" dim=',irank, ', name=',elm_dimnames(irank)
      write(string3,*)'DART "'//trim(varname)//'" dim=',irank, ', name=',dart_dimnames(irank)
      call error_handler(E_ERR, routine, string1, source, text2=string2, text3=string3)
   endif

   varsize(irank) = elm_dimlengths(irank)

enddo DimCheck

if (verbose > 0) then
      write(string1,*)'Replacing indeterminate values ...'
      write(string2,*)'ELM  "'//trim(varname)//'" shape = ', elm_dimlengths(1:elm_numdims)
      write(string3,*)'DART "'//trim(varname)//'" shape = ',dart_dimlengths(1:elm_numdims)
