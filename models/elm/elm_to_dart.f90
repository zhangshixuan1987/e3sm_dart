...
!-------------------------------------------------------------------------------
! purpose: Update ELM output files to be suitable as input to filter
!                 Current functionality:
!                 - replaces random values in empty snow layers with a consistent _FillValue
!
! USAGE:  The elm filename is read from the elm_in namelist
!         <edit elm_to_dart_output_file in input.nml:elm_to_dart_nml>
!         elm_to_dart
!
...
character(len=*), parameter :: source = 'elm_to_dart.f90'

!-------------------------------------------------------------------------------
! namelist variables 
!-------------------------------------------------------------------------------

character(len=256) :: elm_restart_file = 'elm_restart.nc'
integer            :: verbose = 0

namelist /elm_to_dart_nml/ elm_restart_file, verbose
...
call initialize_utilities(progname='elm_to_dart')

! Read the namelist to get the filename.

call find_namelist_in_file("input.nml", "elm_to_dart_nml", iunit)
read(iunit, nml = elm_to_dart_nml, iostat = io)
call check_namelist_read(iunit, io, "elm_to_dart_nml") ! closes, too.

ncid = nc_open_file_readwrite(elm_restart_file, 'open for unused snow layer value replacement')
   ...
   select case (dimnames(1))
      case ( 'levtot', 'levsno')
         if (verbose > 0) then
            ...
            call error_handler(E_MSG,'elm_to_dart',string1)
         endif
...
call finalize_utilities('elm_to_dart')
...
if ( .not. nc_variable_exists(ncid,'SNLSNO') ) then
   write(string1,*)'"'//trim(elm_restart_file)//'" has no "SNLSNO" variable.'
   string2 = 'ELM restart files have "SNLSNO", elm_to_dart only works with restart files.'
