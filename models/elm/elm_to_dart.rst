...
``elm_to_dart`` is used to replace indeterminate values in the ELM restart file
...
variable that may be part of the DART state. Only ELM restart files have variables
that require this treatment.

``elm_to_dart`` reads every variable in the input file and determines if the variable 
...
The issue arises because all ELM columns have a fixed number of snow layers but only
some of the layers may actually contain snow. ELM internally tracks the number of
...
``elm_to_dart`` **overwrites** the input file.
Consequently, the intended use is to **copy** the ELM restart file and feed the copy 
into ``elm_to_dart``. This strategy allows the copies to be read into DART and updated 
with the posterior values. The companion :doc:`dart_to_elm` uses the *_FillValue* 
values as a mask to only update the original ELM restart file with valid posterior 
values. Where the DART posterior file has *_FillValue* values, the original ELM 
...
ELM restart files need the treatment. ELM history files do not have *SNLSNO*.

It is a useful exercise to dump the ELM restart file and the output of ``elm_to_dart``
...
.. container:: unix
   ...
   :: 
      ...
      cp T_SOISNO elm_restart.nc elm_restart.nc
      ncdump -f F -v T_SOISNO elm_restart.nc > T_SOISNO_org.txt
      ./elm_to_dart
      ncdump -f F -v T_SOISNO elm_restart.nc > T_SOISNO_new.txt
...
::
   ...
   &elm_to_dart_nml
     elm_restart_file  = 'elm_restart.nc'
     verbose           = 0
    /
...
.. container::
   ...
   ================== ==================== ========================================================================== 
   elm_restart_file   character(len=256)   Path name of the ELM restart file to be overwritten.
...
-  elm_restart_file, specified by namelist;
...
using the appropriate low-resolution ELM case.
The ``elm_to_dart_nml`` verbosity was set to 3 and the ``elm_to_dart``  run-time output was captured 
...
.. note::
 ...
 The program elm_to_dart identifies indeterminate values for any variable that
...
In the example below, the execution of ``elm_to_dart`` will overwrite the indeterminate
values within the elm restart file with the *_FillValue* attribute value to prevent DART
...
:: 
  ...
  There are          520  variables in the file.
   ...
   elm_to_dart  variable #          204  is "T_SOISNO" - replacing indeterminate values
