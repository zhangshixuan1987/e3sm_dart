...
! This is the interface between the E3SM Land Model (ELM) and DART.
...
public :: get_gridsize,                 &
          get_elm_restart_filename,     &
          compute_gridcell_value,       &
          gridcell_components

character(len=*), parameter :: source   = 'elm/model_mod.f90'
...
!
!  The variables in the elm restart file that are used to create the
...
character(len=256) :: elm_restart_filename = 'elm_restart.nc'
character(len=256) :: elm_history_filename = 'elm_history.nc'
character(len=256) :: elm_vector_history_filename = 'elm_vector_history.nc'
...
!------------------------------------------------------------------------------
! Things that come from the ELM history file.
!
...
!------------------------------------------------------------------------------
! Things that come from the ELM restart file.
!
...
!---------------------------------------------------------------
! The ELM history file (h0?) has the 'superset' of information.
! The ELM restart files are intentionally lean and, in so doing,
! do not have the full lat/lon arrays nor any depth information.
!
ncid = 0; ! signal that netcdf file is closed
call get_history_dims(ncid, elm_history_filename, 'open', nlon, nlat, &
...
call get_elm_codes(ncid, elm_history_filename, 'open')
call get_full_grid(ncid, elm_history_filename, 'close')

ncid = 0; ! signal that netcdf file is closed

!---------------------------------------------------------------
! The ELM grid in a restart file is fundamentally a sparse matrix
...
call get_sparse_geog(ncid, elm_restart_filename, 'close')
...
!---------------------------------------------------------------
! Compile the list of elm variables to use in the creation
! of the DART state vector.

call parse_variable_table( elm_variables, nfields, variable_table )
...
if (nvars > 0) then
   dom_restart = add_domain(elm_restart_filename, nvars, var_names(1:nvars),  &
...
call cluster_variables(variable_table, 'HISTORY', nvars, var_names, &
                       var_qtys, var_ranges, var_update)
if (nvars > 0) then
   dom_history = add_domain(elm_history_filename, nvars, var_names(1:nvars), &
                  kind_list   = var_qtys(  1:nvars),   &
                  clamp_vals  = var_ranges(1:nvars,:), &
                  update_list = var_update(1:nvars) )
...
call cluster_variables(variable_table, 'VECTOR', nvars, var_names, &
                       var_qtys, var_ranges, var_update)
if (nvars > 0) then
   dom_vector = add_domain(elm_vector_history_filename, nvars, var_names(1:nvars), &
                  kind_list   = var_qtys(  1:nvars),   &
                  clamp_vals  = var_ranges(1:nvars,:), &
                  update_list = var_update(1:nvars) )
...
call nc_add_global_attribute(ncid, "model", "ELM")
...
!-------------------------------------------------------------------------------
!> Actually, nothing special about get_close_obs for ELM. Including it here
...
!> This (required interface) is unsupported in ELM and any attempt
...
!------------------------------------------------------------------
!> reads the valid time of the ELM model state from the restart file
...
select case( obs_kind )
   ...
   case ( QTY_SOIL_MOISTURE )
      ...
      ! only elm (history) variable 'H2OSOI' is actually soil moisture.
      ...
      ! This is terrible ... the COSMOS operator wants m3/m3 ... ELM is kg/m2
...
if ( loc_lev < 0.0_r8 ) then
   ...
   write(string3,*)'ELM has negative depths to indicate above-ground values.'
...
!------------------------------------------------------------------
!> Read the grid dimensions from the ELM history netcdf file.
...
subroutine get_elm_codes(ncid, fname, cstat)

integer,          intent(inout) :: ncid
character(len=*), intent(in)    :: fname
character(len=*), intent(in)    :: cstat

character(len=*), parameter :: routine = 'get_elm_codes'
integer :: ret

if (ncid == 0) ncid = nc_open_file_readonly(fname, routine)

! If the read is successful, use it - if not, use the default from ELM 4.5
...
end subroutine get_elm_codes


!------------------------------------------------------------------
!> Read the dimensions from the ELM restart netcdf file.
...
! ELM4 does not have a multi-level canopy.
! ELM4.5 has a multi-level canopy.
...
subroutine get_elm_restart_filename( filename )

character(len=*), intent(OUT) :: filename

if ( .not. module_initialized ) call static_init_model

filename = trim(elm_restart_filename)

end subroutine get_elm_restart_filename
...
!>  specified by the input.nml:model_nml:elm_variables  variable.
...
!>     only variables marked '.r', 'UPDATE' will be modified for ELM.
!>
...
MyLoop : do i = 1, nrows
   ...
   if ( any(table(i,:) == ' ') ) then
      string1 = 'input.nml &model_nml:elm_variables not fully specified'
...
!------------------------------------------------------------------
!> icol            ... which ELM 'column' are we in
...
!> (a better version is in the elm/matlab/CheckStorageOrder.m function)
!>
...
!> the model_time from the ELM restart file.
