...
This tutorial describes how to set up a simple assimilation using ELM-DART. The
expectation is that the user is very familiar with running ELM off-line
and thus the tutorial focuses upon the specific aspects of integrating ELM
with DART. Once completed, the goal is that the user should be familar enough
with the concepts of ELM-DART in order to design an assimilation for their own
research interests.

This tutorial was assembled to be compatible with ELM5.0.34 as part
...
Other combinations of ELM and DART (prior to DART version 9.13.0) may not be compatible
...
with running and understanding CLM. If new to ELM we recommend you
attend a `CLM tutorial <https://www.cesm.ucar.edu/events/tutorials/>`__
or review the ELM documentation including the 
...
This ELM-DART tutorial is based on a simple example in which we 
describe how to:


1. set up the software
2. review concepts of assimilation 
3. provide a specific example of how ELM and DART interact in an assimilation framework 
...
modify the ELM-DART setup.  In general, an ELM-DART simulation that is running
...
We encourage users to complete the tutorial and then modify ELM-DART to pursue their own
...
ensemble (i.e. 5 ELM simulations run simultaneously). An ensemble
is required within Ensemble Kalman Filter (EnKF) data assimilation to calculate
the modeled covariance amongst the state variables in ELM.
The covariance amongst the ELM state variables dictates how
the ELM model state is updated during the assimilation step given
...
a set of ELM restart files generated from a previous CLM 5-member
...
spread within this ELM-DART tutorial simulation.

.. NOTE::

  This ELM-DART tutorial has been simplified to reduce run-time and
...
The goal of this tutorial is to demonstrate how ELM and DART
interact to perform data assimilation to provide an
observation-constrained simulation of ELM. This requires
setting numerous namelist/input values that control the 
configuration of ELM and DART setup. 
...
However, you will need to do additional work to customize the ELM-DART
...
.. Important ::
  ...
  text editors (e.g. vi, nedit, emacs) to edit the ELM-DART setup scripts 
  ...
  2. `Reference Case and Observations <https://www.image.ucar.edu/pub/DART/CESM/clmdart_tutorial/>`__,
     ...
     ``elm_dart_tutorial_2022_03_01.tar.gz``
...
Adding ELM SourceMods
----------------------

Some minor modifications have to be made to the ELM source code in order
to be run with DART. Most importantly, these include skipping several
balance checks in ELM for the time step immediately after the assimilation
...
SourceMods see the main :doc:`ELM-DART documentation. <../readme>`

Compiling CLM5
--------------

Compiling ELM on the NSF NCAR machine Derecho is straightforward because the 
...
When performing a ELM-DART assimilation run, the compiling step for ELM occurs within
...
Below is a table of the key ELM-DART setup scripts that include the 
settings required to perform an  assimilation run. All scripts are 
located at path ``DART/models/elm/shell_scripts/cesm2_2/`` with the 
exception of ``input.nml`` which is located at ``DART/models/elm/work/``.
...
For additional description of the ELM-DART scripts and concepts please
visit the main :doc:`ELM-DART documentation <../readme>`
In general, feel free to supplement this tutorial with the main ELM-DART documentation. 
...
+-------------------------+--------------------------------------------------------------+
|  Key ELM-DART scripts   |    Description                                               |
+=========================+==============================================================+
| CLM5_setup_assimilation | Main setup script that creates a ELM-DART assimilation      |
...
|                         | that defines important case settings for ELM and DART.       |
...
|                         | assimilation by providing links between the ELM and DART     |
|                         | code. Converts a 'free' run into an assimilation run.        |
+-------------------------+--------------------------------------------------------------+
| assimilate.csh          | This script is executed during the assimilation case         |
|                         | run-time. It orchestrates communication between ELM and DART.|
|                         | After the ELM forecast step is complete, it calls upon the   |
|                         | DART executables to relay the ELM files (restart, history)   |
|                         | to DART and executes the ``filter`` step to update the ELM   |
|                         | state variable. These updated files are then reinserted      |
|                         | into the restart file for the next ELM forecast step.        |
+-------------------------+--------------------------------------------------------------+
...
Similar to ELM, it is necessary to compile the DART code before an assimilation
...
observations that we wish to assimilate into ELM.  For more information
...
``obs_quantity`` commonly used for land DA, including models like ELM.
...
and 2) correlation between state variables.  Given the sensitivity of ELM to 
atmospheric conditions an established method to generate multi-instance ELM
...
is correct. In particular make sure the ``SOURCEDIR`` variable is set correctly below:  
 ...
 ::
   ...
   set SOURCEDIR = ${dartroot}/models/elm/shell_scripts/cesm2_2
...
.. Note::
   ...
   changes within the ELM variables that you plan to adjust during the DART update step. 
   ELM variables that have relatively quick response to atmospheric forcing (e.g. leaf area,
   shallow-depth soil variables) require less spinup time.  However, other ELM variables
...
ELM 'hybrid' mode which allows the starting date of the assimilaton to be different than
...
|                             | uses to update the ELM model.  This is derived from the     |
|                             | true observation value generated from ELM model output with |
|                             | uncertainty added.                                          |
+-----------------------------+-------------------------------------------------------------+
| true observation value      | The observation generated from ELM output.  In this case    |
...
|                             | is calculated from the ELM model ensemble mean, and the     |
...
.. Note::
   ...
   land DA and the ELM model include those for :doc:`leaf area, <../../../observations/obs_converters/MODIS/MOD15A2_to_obs>`
...
Step 8: Setting up the DART and ELM states 
------------------------------------------
...
``MODIS_LEAF_AREA_INDEX``, and ``BIOMASS`` are supported by specific ELM variables. See the table
below which defines the dependency of each DART **observation type** upon specific DART **quantities** 
required for the forward operator. We also include the ELM variables that serve as the DART
quantities for this tutorial:


+--------------------------+-----------------------------+-----------------+
| DART Observation Type    | DART Observation Quantities | ELM variables   |
+==========================+=============================+=================+
...
.. Note::
  ...
  (and ELM variable) to calculate the expected observation.  For these the ELM
  ...
  to calculate the expected observation.  In that case the sum of the ELM
  variables of leaf, live stem and structural (dead) carbon represents the biomass observation. 
 

Second, the DART state space also defines which portion of the ELM model state is updated by DART. 
In DA terminology, limiting the influence of the observations to a subset of the ELM model
state is known as 'localization' which is discussed more fully in Step 9.
In theory the complete ELM model state may be updated based on the relationship with the observations.
...
the update to ELM variables most closely related to biomass, leaf area, soil temperature and
...
The table below provides a description for each of the columns for ``elm_variables`` within
``&model_nml``.
...
There are **important** distinctions about the ``elm_variables`` as described above. 
**First**, any ELM variable whether it is a ``restart``, ``history`` or ``vector`` file can be used
...
regardless of the ELM variable type. **However**, in order for the update step to have a
permanent effect upon the evolution of the ELM model state, the update must be applied to a
prognostic variable in ELM -- which is always the ``restart`` file.  Updates to ``restart``
file variables alters the file thus changing the initial conditions for the next time
step.  The ELM ``history`` and ``vector`` files, on the other hand, are diagnostic variables
with no impact on the evolution of the model state.

A **second** important distinction amongst ``elm_variables`` is that the ``restart`` file
state variables are automatically generated after each ELM simulation time step, thus are readily
available to include within the DART state. In contrast, the ``history`` or ``vector`` file variables
must be manually generated through the ``user_nl_elm`` file within ELM.  This is generated
...
``fincl3->h2``) for each of the ELM variables as defined above. The 
...
the state update to a subset of ELM variables.  Here, we further restrict the influence
...
the model update. For ELM-DART assimilations the ensemble spread is generated through
...
step similar to ELM state variables.  The inflation state properties include 
...
::
 ...
 &fill_inflation_restart_nml
   input_state_files = 'elm_restart.nc','elm_history.nc','elm_vector_history.nc'
   single_file       = .false.
...
The ``assimilate.csh`` script assigns the ELM file that defines each domain. In this tutorial
...
::
 ...
 set     LND_RESTART_FILENAME = ${CASE}.elm2_0001.r.${LND_DATE_EXT}.nc
 set     LND_HISTORY_FILENAME = ${CASE}.elm2_0001.h0.${LND_DATE_EXT}.nc
 set LND_VEC_HISTORY_FILENAME = ${CASE}.elm2_0001.h2.${LND_DATE_EXT}.nc
...
::
 ...
 ls -1         elm2_*.r.${LND_DATE_EXT}.nc  >! restart_files.txt
 ls -1 ${CASE}.elm2_*.h0.${LND_DATE_EXT}.nc >! history_files.txt
 ls -1 ${CASE}.elm2_*.h2.${LND_DATE_EXT}.nc >! vector_files.txt
...
::
 ...
 cd DART/models/elm/shell_scripts/cesm2_2/
...
run.  This means if the case is submitted as is, it will perform a normal ELM 
...
::
 ...
 DATA_ASSIMILATION_SCRIPT= <dartroot>/models/elm/shell_scripts/cesm2_2/assimilate.csh
 STOP_OPTION=nhours
 STOP_N=24
...
|                             | files between DART and ELM code.                            |
+-----------------------------+-------------------------------------------------------------+
...
.. Tip::
 ...
 executables to be re-compiled within ``<dartroot>/models/elm/work``.  Executing ``stage_dart_files``
...
2) The ELM posterior member values are updated from their prior values
...
an example of a successful update (``elm_obs_seq.2011-01-02-00000.final``)  which is
...
Below we provide the variable names for the ``elm_obs_seq.2011-01-02-00000.final``
example from above.
...
Now, compare your ``elm_obs_seq.final`` file to the example provided above:

::

  cd <rundir>
  less elm_obs_seq.2011-01-02-00000.final
...
Do not expect your own ``elm_obs_seq.final`` file to be bit-by-bit identical 
...
small changes when running ELM-DART.  Furthermore, ELM is set up
...
.. Note::
 ...
 scripts can be found here (``DART/models/elm/matlab/README.txt``).
...
the observations and the expected observation (derived from the ELM state),
...
ELM state.  A successful assimilation reduces the RMSE, thus reducing the mismatch
between the observations and the ELM state. Finally the total spread provides
...
Next generate a text file that includes all the ``elm_obs_seq*.final``
...
to the text file containing the names of the ``elm_obs_seq*.final``
...
Next convert the information in the ``elm_obs_seq*.final`` files into
...
::
 ...
 module load matlab
 matlab -nodesktop
 >> fname   = '<dartroot>/models/elm/work/obs_diag_output.nc';
 >> copy    = 'totalspread';
...
For this tutorial we plot a list of the  ``elm_obs_seq*.final`` files as shown below,
...
::
 ...
 module load matlab
 matlab -nodesktop
 >> fname         = '<dartroot>/models/elm/work/obs_epoch_001.nc';
...
your way to designing ELM-DART assimilations for your own research.
