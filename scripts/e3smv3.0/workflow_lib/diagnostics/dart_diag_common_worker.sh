...
if [ ${my_eam_dart_diag_run_obs2netcdf}  = '.true.' ];then
  ...
  OBSSEQ_DIR="${CURRENT_DADIR}/cam6_common"
  ...
  ${LIST} ${OBSSEQ_DIR}/${DART_CASE}.dart.e.${scomp}_obs_seq.processed.*.common > ${obs_sequence_list}
...
if [[ ${my_eam_dart_diag_run_obs_diag}  == '.true.' ]];then
  ...
  OBSDIAG_DIR="${CURRENT_DADIR}/cam6_common"
  ...
  find "${OBSDIAG_DIR}" -type f -name "${DART_CASE}.dart.e.${scomp}_obs_seq.processed.*.common" -print | awk -F. -v start="${my_eam_dart_diag_start}" -v end="${my_eam_dart_diag_end}" '{stamp=$(NF-1); if (stamp >= start && stamp <= end) print}' | sort > "${obs_sequence_list}"
  [ -s "${obs_sequence_list}" ] || { echo "ERROR: no common observation sequences in diagnostic range" >&2; exit 142; }
